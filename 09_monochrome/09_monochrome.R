...
nodes <- airport[,c(5,7,8)] %>% distinct(X5,.keep_all = TRUE) %>% dplyr::filter(!is.na(X5))# %>% mutate(X1=paste0("A",X1))
names(nodes) <- c("name","lat","long")
edges <- routes[,c(3,5)] %>% dplyr::filter(!is.na(X3),!is.na(X5)) #%>% mutate(X4=paste0("A",X4),X6=paste0("A",X6))
...
idx <- pbundle %>% group_by(group) %>% dplyr::summarise(abs=max(x)-min(x)) %>% arrange(-abs) %>% pull(group) %>% .[1:86]
p <- ggplot()+
  ...
  labs(title="Global Flight Network",
...
world1 <- world
pbundle_sf1 <- pbundle_sf[-idx,]
world1 <- st_transform(world1,"+proj=robin")
pbundle_sf1 <- st_transform(pbundle_sf1,"+proj=robin")
world1 <- st_cast(world1,"POLYGON")

world1$fill="#fbedee"
world1$color="#f0b8be"

pbundle_sf1$color <- "#db4f5d"
pbundle_sf1$stroke <- 0.05

roughsf::roughsf(list(world1,pbundle_sf1),simplification = 0.01,height=2500,width=3500)
