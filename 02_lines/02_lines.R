...
dict <- dict %>%
  left_join(centers, by = "iso_a3") %>%
  mutate(
    x = str_extract(the_geom, "\\([\\-0-9.]+") %>% str_remove("\\(") %>% as.numeric(),
...
p <- ggplot() +
  ...
  theme_minimal() +
  theme(
    panel.background = element_rect(color = NA, fill = "grey99"),
  ...
  labs(
    title = "Global Migration (2005-2010)",
    subtitle = "colored by continent of origin",
    caption = "@schochastics"
  ) +
  annotate("text",
    y = -Inf, x = 0, label = '(Data from Abel and Sander "Quantifying Global International Migration Flows")',
