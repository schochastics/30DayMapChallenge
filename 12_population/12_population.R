...
pbundle_sf$dir_ew <- ambient::normalise(xy[el[, 1], 1] - xy[el[, 2], 1], 
                                        from = c(-57.8431, 57.8431), to = c(-1, 1))
pbundle_sf$dir_ns <- ambient::normalise(xy[el[, 1], 2] - xy[el[, 2], 2], 
                                        from = c(-24.8269, 24.8269), to = c(-1, 1))

pbundle_sf$dir_ew_d <- case_when(
  pbundle_sf$dir_ew < -0.33 ~ "we",
...
flow_tbl <- tibble(name = V(g)$name, inflow = graph.strength(g, mode = "in"), 
                   outflow = graph.strength(g, mode = "out"), 
                   netflow = (inflow + 1) / (outflow + 1))
...
pbundle_sf1$color <- case_when(
  pbundle_sf$dir_ew < -0.1 ~ "#832424",
...
legend_text <- tibble(
  label = c("from west to east", "from east to west", "inflow/outflow = min (more outflow)", "inflow/outflow = max (more inflow)"),
  label_pos = c("e", "e", "e", "e"),
  size = 0.0, color = "#ffffff"
)
leg_geom <- st_sfc(
  st_point(c(midx - 5.2, ypos)), st_point(c(midx + 11.8, ypos)),
