...
#colors
cols_lvls <- c("#252525", "#636363", "#969696", "#CCCCCC", "#08519C", "#3182BD", 
...
urca <- st_as_sf(x, as_points = FALSE, merge = TRUE)#st_as_sf(x, as_points = TRUE, merge = FALSE)
saveRDS(urca,"urban_shp.RDS")
urca <- readRDS("urban_shp.RDS")
names(urca)[1] <- "lev"
urca$lev <- round(urca$lev)
urca$col <-  case_when(urca$lev%in%c(1,2)     ~"Large city (>1 mil.)",
...
ggplot()+
  ...
  # geom_sf(data=urca,aes(col=as.factor(col)))+
  ...
  labs(title = "Urban–Rural Catchment Areas",
...
# #cols
# #large
# col_large <- c("#252525","#252525","#636363","#636363","#969696","#969696","#CCCCCC","#CCCCCC")
# id_large <- c(1,2,8,9,15,16,22,23)
# #intermed
# col_inter <- c("#08519C","#08519C", "#3182BD", "#3182BD","#6BAED6","#6BAED6", "#BDD7E7", "#BDD7E7" )
# id_inter <- c(3,4,10,11,17,18,24,25)
# #small
# col_small <- c("#E6550D", "#E6550D", "#E6550D", "#FD8D3C", "#FD8D3C", "#FD8D3C", 
#                "#FDBE85", "#FDBE85", "#FDBE85", "#FEEDDE", "#FEEDDE", "#FEEDDE")
# id_small <- c(5,6,7,12,13,14,19,20,21,26,27,28)
# 
# #disperesed
# col_disp <- "#31a354"
# id_disp <- 29
